...
onSaveIdea() {
  const formData = this.ideaSaveForm.value;

  // Modify formData to handle optional fields
  if (!formData.fileSolutionPath || formData.fileSolutionPath.length === 0) {
    formData.fileSolutionPath = ['Null'];  // Use an empty array or null based on backend requirements
  }
  if (!formData.fileSituationPath || formData.fileSituationPath.length === 0) {
    formData.fileSituationPath = ['Null'];  // Use an empty array or null based on backend requirements
  }
  if (!formData.descriptionSolution) {
    formData.descriptionSolution = '';  // Use an empty string or null based on backend requirements
  }
  if (!formData.descriptionSituation) {
    formData.descriptionSituation = '';  // Use an empty string or null based on backend requirements
  }

  if (this.ideaSaveForm.valid) {
    console.log("Valid form data:", formData);

    this.proposeIdea.saveIdea(formData).subscribe({
      next: (res) => {
...
globalSearch() {
  ...
  this.Ideas = this.Ideas.filter((idea: any) => {
    return Object.values(idea).some((value: any) =>
      // String(idea.buildingID.buildingID).toLowerCase().includes(searchTerm) ||

      String(value).toLowerCase().includes(searchTerm)
    );
  });
  console.log('Filtered ideas:', this.Ideas);
 ...
 this.userStore.getIdFromStore().pipe(
  ...
  console.log('Filtered committees:', this.proposeIdea);
